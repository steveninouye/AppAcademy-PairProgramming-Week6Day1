...
classes/sourcefiles. All of the following will be located in the `src` directory which we will talk about in a moment:
...
In the `index.js`, add an event listener for the [`DOMContentLoaded`][dom-content-loaded] event. Within the
...
context. Now that you're using a `DOMContentLoaded` callback you can safely move your `main.js` script tag in `index.html` into the `<head>` where it belongs.

This is your webpack entry point, so start webpack (or restart it, if you're
already running the webpack command provided above) with the "index.js" file as your new entry
point.
...
In `src/ship.js`, write an `Ship` class; this should be another subclass of
...
`main.js` file. The `keymaster` library will expose a global method `key`
